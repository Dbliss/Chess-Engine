function [bestMove, bestValue, moveSequence, booked] = engine2(board, colour, positionValue, depth, alpha, beta, moveSequence, prevBest, hashTable, zKeys, book, bookTables, recursiveCap)
    booked = false;
    ...
    for i = 1:size(legalMoves, 2)
        ...
        if legalMoves(4, i) < 0 && depth >= 2
            if (legalMoves(5, i) == 3 && recursiveCap < 4)
                [~, childValue, ~, ~] = engine2(newBoard, other_colour, positionValue, depth - 1, -beta, -alpha, newMoveSequence, inp, hashTable, zKeys, book, bookTables, recursiveCap + 1);
            else
                [~, childValue, ~, ~] = engine2(newBoard, other_colour, positionValue, depth - 2, -beta, -alpha, newMoveSequence, inp, hashTable, zKeys, book, bookTables, recursiveCap);
            end
        else
            if (legalMoves(5, i) == 3 && recursiveCap < 4)
                [~, childValue, ~, ~] = engine2(newBoard, other_colour, positionValue, depth, -beta, -alpha, newMoveSequence, inp, hashTable, zKeys, book, bookTables, recursiveCap + 1);
            else
                [~, childValue, ~, ~] = engine2(newBoard, other_colour, positionValue, depth-1, -beta, -alpha, newMoveSequence, inp, hashTable, zKeys, book, bookTables, recursiveCap);
            end
        end
